...
def visualize_dataset(manifest_csv: str, weights_path: str, num_samples: int = 5, save_dir: str = "visualizations"):
    df = pd.read_csv(manifest_csv)
    os.makedirs(save_dir, exist_ok=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Loading Custom Weights from [{weights_path}] on [{device}]...")
    ...
    for idx in sample_indices:
        ...
        axes[0].set_title(f"Sample #{idx} | Reference Crop")
        axes[0].axis('off')

        axes[1].imshow(search_rgb)
        axes[1].set_title(f"Search Field (Custom Siamese) | Error: {error_px:.2f} px")
        axes[1].axis('off')

        plt.tight_layout()
        save_path = os.path.join(save_dir, f"sample_{idx}_custom_siamese.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights", type=str, default="siamese_wafer_epoch_10.pth")
    parser.add_argument("--num_samples", type=int, default=5)
    parser.add_argument("--save_dir", type=str, default="visualizations")

    args = parser.parse_args()
    visualize_dataset(args.manifest, args.weights, args.num_samples, args.save_dir)